...
def val(model, val_loader):
    ...
    return OrderedDict({'Val Loss': val_loss, 'Val dice0': val_dice0,'Val dice1': val_dice1})


def train(model, train_loader, optimizer, trainsavepath,trainnum):
    ...
    for idx, (data, target) in tqdm(enumerate(train_loader),total=len(train_loader)):
        ...
        '''if idx<len(train_loader)-1:
            ...
            for i in range(trainnum%args.batch_size):
                ...
                img3 = sitk.GetImageFromArray(c[i][0])
                sitk.WriteImage(img3, os.path.join(trainsavepath, str(idx) + '-' + str(i) + '-rawdata.tiff'))'''
        #if idx==0 or idx==500 or idx==1000 or idx==1300:
        if idx%100==0:
            for i in range(args.batch_size):
                img = sitk.GetImageFromArray(a[i][1])
                sitk.WriteImage(img, os.path.join(trainsavepath, str(idx) + '-' + str(i) + '-edgeresult.tiff'))
                img2 = sitk.GetImageFromArray(b[i][1])
                sitk.WriteImage(img2, os.path.join(trainsavepath, str(idx) + '-' + str(i) + '-edgelabel.tiff'))
                img3 = sitk.GetImageFromArray(a[i][0])
                sitk.WriteImage(img3, os.path.join(trainsavepath, str(idx) + '-' + str(i) + '-bodyresult.tiff'))
                img4 = sitk.GetImageFromArray(b[i][0])
                sitk.WriteImage(img4, os.path.join(trainsavepath, str(idx) + '-' + str(i) + '-bodylabel.tiff'))
...
if __name__ == '__main__':
    ...
    model = unet3d(1, [8,16,32,64,128],2).to(device) #2021.10.8
    ...
    trainsavepath=r'D:/A_DLcsz/result'
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_log = train(model, train_loader, optimizer, trainsavepath,trainnum)
        #print("lr:", scheduler.get_lr())
